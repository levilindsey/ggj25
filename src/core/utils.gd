...
static func transform_screen_position_to_level_position(
    ...
    return (G.level.get_canvas_transform() * \
            G.level.get_global_transform()) \
        .affine_inverse() * \
        (screen_position - G.screens.get_top_screen().screen.rect_position)
...
static func get_screen_position_of_node_in_level(node: CanvasItem) -> Vector2:
    return node.get_global_transform_with_canvas().origin + \
        G.screens.get_top_screen().screen.rect_position
...
static func mix(
    ...
    if G.utils.is_num(values[0] is float):
...
static func get_type_string(type: int) -> String:
    match type:
        ...
        TYPE_MAX:
            return "TYPE_MAX"
        _:
            G.log.error("Utils.get_type_string: %d" % type)
            return ""
